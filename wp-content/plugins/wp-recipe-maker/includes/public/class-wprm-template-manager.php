...
class WPRM_Template_Manager {
	...
	public static function templates_css() {
		if ( 'modern' === WPRM_Settings::get( 'recipe_template_mode' ) && count( self::$used_templates ) ) {
			$style = '';
	...
	public static function get_template( $recipe, $type = 'single', $slug = false ) {
		...
		$wprm_template_output = preg_replace( "/\[wprm-recipe\s+id=\"?'?" . $recipe->id() . "\"?'?\]/im", '', $wprm_template_output );
		...
		if ( 'print' === $type ) {
			...
			if ( preg_match_all( '/' . $pattern . '/s', $wprm_template_output, $matches ) && array_key_exists( 2, $matches ) ) {
			...
			foreach ( $label_shortcodes as $shortcode => $shortcode_options ) {
				...
				if ( ! $recipe_id ) {
					...
					$wprm_template_output = str_replace( $shortcode, $new_shortcode, $wprm_template_output );
				}
			}
		}
