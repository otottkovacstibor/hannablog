                ...
                <td>
                    <span class="wpurp_adding">
                        ...
                        if( !in_array( $taxonomy->name, $this->protectedTaxonomies ) ) {
                            submit_button( __( 'Delete', 'wp-ultimate-recipe' ), 'delete', 'submit-delete-' . $taxonomy->name, false );
                        }
                        ?>
                    </span>
                </td>
...
<form method="POST" action="<?php echo admin_url( 'admin.php' ); ?>" onsubmit="return confirm('<?php _e('Do you really want to reset the taxonomies? All custom tags will be deleted.', 'wp-ultimate-recipe' ); ?>');">
    <input type="hidden" name="action" value="reset_taxonomies">
    <?php wp_nonce_field( 'reset_taxonomies', 'reset_taxonomies_nonce', false ); ?>
    <?php submit_button( __( 'Reset to defaults', 'wp-ultimate-recipe' ) ); ?>
</form>